...
class FinancialAnalysisDetailPage extends StatelessWidget {
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Obx(() {
                ...
                final nextTransactionPrediction =
                    _calculateNextTransactionPrediction(controller.transaction);
  ...
  Widget _buildPredictionItem(Map<String, dynamic> item, ThemeData theme) {
    return Container(
      margin: EdgeInsets.only(bottom: 12.h),
      padding: EdgeInsets.all(12.w),
      decoration: BoxDecoration(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            _formatCurrency(item['valorPrevisto']),
                            style: TextStyle(
                              fontSize: 12.sp,
                              fontWeight: FontWeight.bold,
  ...
  List<Map<String, dynamic>> _calculateNextTransactionPrediction(
      List<TransactionModel> transactions) {
    List<Map<String, dynamic>> predictions = [];

    // Separar receitas e despesas
    final receitas =
        transactions.where((t) => t.type == TransactionType.receita).toList();
    final despesas =
        transactions.where((t) => t.type == TransactionType.despesa).toList();
    ...
    // Calcular previsão para receitas
    if (receitas.length >= 3) {
      final last3Receitas = receitas.take(3).toList();
      final avgReceita = _calculateAverageFromTransactions(last3Receitas);

      predictions.add({
        'title': 'Próxima Receita Prevista',
        'description': 'Baseado nas últimas 3 receitas',
        'valorPrevisto': avgReceita,
        'tipo': 'receita',
        'baseadoEm': 3,
      });
    }

    // Calcular previsão para despesas
    if (despesas.length >= 3) {
      final last3Despesas = despesas.take(3).toList();
      final avgDespesa = _calculateAverageFromTransactions(last3Despesas);

      predictions.add({
        'title': 'Próxima Despesa Prevista',
        'description': 'Baseado nas últimas 3 despesas',
        'valorPrevisto': avgDespesa,
        'tipo': 'despesa',
        'baseadoEm': 3,
      });
    }

    // Se não há 3 transações, usar as disponíveis
    if (receitas.length < 3 && receitas.isNotEmpty) {
      final avgReceita = _calculateAverageFromTransactions(receitas);
      predictions.add({
        'title': 'Próxima Receita Prevista',
        'description': 'Baseado em ${receitas.length} receita(s)',
        'valorPrevisto': avgReceita,
        'tipo': 'receita',
        'baseadoEm': receitas.length,
      });
    }

    if (despesas.length < 3 && despesas.isNotEmpty) {
      final avgDespesa = _calculateAverageFromTransactions(despesas);
      predictions.add({
        'title': 'Próxima Despesa Prevista',
        'description': 'Baseado em ${despesas.length} despesa(s)',
        'valorPrevisto': avgDespesa,
        'tipo': 'despesa',
        'baseadoEm': despesas.length,
      });
    }
  ...
  double _calculateAverageFromTransactions(
      List<TransactionModel> transactions) {
